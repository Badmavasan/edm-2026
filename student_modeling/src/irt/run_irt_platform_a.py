#!/usr/bin/env python3
"""IRT training for Platform A dataset.
...
Run with: python src/irt/run_irt_platform_a.py
"""
...
def load_dataset(path: Path) -> pd.DataFrame:
    """Load and preprocess the Platform A dataset."""
    ...
    for n_samples in tqdm(sample_sizes, desc="Incremental (samples)", leave=False):
        ...
        try:
            ...
            sample_results.append({
                ...
                "train_students": n_students_in_slice,
                "test_samples": len(y_true),
                "auc": auc,
                "accuracy": accuracy_score(y_true, y_pred),
                "f1": f1_score(y_true, y_pred, zero_division=0),
...
def main():
    print("=" * 70)
    print(" IRT Training - PLATFORM A Dataset")
    print("=" * 70)

    base_path = Path(__file__).parent.parent.parent
    data_path = base_path / "data" / "platform_a_dataset.csv"
    output_dir = base_path / "results" / "platform_a" / "irt"
    ...
    if len(global_incremental_data) >= 1:
        plot_incremental_auc(
            ...
            "IRT: AUC vs Training Data Size (Global)"
        )
        # Also save to root
        plot_incremental_auc(
            global_incremental_data,
    ...
    for ex_type in tqdm(exercise_types, desc="Exercise Types"):
        subset_df = wide_df[wide_df[EXERCISE_TYPE_COL] == ex_type].copy()
        ex_type_roc_data = []
        ...
        for modality in MODALITIES:
            ...
            modality_dir = output_dir / "exercise_type" / modality / str(ex_type)
        ...
        if len(ex_type_roc_data) >= 1:
            plot_roc_comparison(
                ex_type_roc_data,
                output_dir / "exercise_type" / f"roc_comparison_{ex_type}.png",
    ...
    if exercise_type_results:
        pd.DataFrame(exercise_type_results).to_csv(
            output_dir / "per_exercise_type_accuracy.csv", index=False
        )
    ...
    # ==========================================================================
    print("\n" + "=" * 70)
    print(" IRT Training Complete - PLATFORM A")
    print("=" * 70)
